...
#         hashed_otp = generate_password_hash(otp)  # Hash the OTP
#         print(hashed_otp)
#         # Check if a user already exists with the given email
#         existing_user = OTPS.query.filter_by(email=user_email).first()

#         if existing_user:
#             existing_user.otp_code_hash = hashed_otp  # Update the OTP hash
#             existing_user.created_at = datetime.utcnow()  # Update the creation timestamp
#         else:
#             # Create a new OTP entry
#             new_otp = OTPS(email=user_email, otp_code_hash=hashed_otp)
...
#     if not token:
#         return jsonify({'message': 'Authentication token is missing. Please login.'}), 401
